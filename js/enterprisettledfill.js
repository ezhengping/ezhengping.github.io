	function form() {
		...
		//验证是否为空
		function Null(nullCls) {
			var val = $(nullCls + ' input').val(); //获取value;
			if (val != '') {
				$(nullCls + ' .input').css({
					'box-shadow': 'none',
					'border-color': '#bfbfbf'
				...
				$(nullCls + ' i').html('内容不能为空').css('display', 'block');
				$(nullCls + ' .input').css('border-color', 'red');
				return false;
			}
		}
		...
		function thirdLink(i) {
			...
			var reg = /[a-zA-z]+:\/\/[^\s]*/;
			if (reg.test(val)) {
				$('.settled_thirdlink .input').eq(i).css({
					'box-shadow': 'none',
					'border-color': '#bfbfbf'
				});
		...
		$('form').on('focus', 'input.input', function () {
			$(this).css({
				'box-shadow': '0 0 6px rgba(255,127,38,.5)',
				'border-color': '#ff7f26'
			});
		})
		// input失去到焦点时
		...
		// 
		// 企业法人名称
		$('.settled_legalperson .input').focus(function () {
			$('.settled_legalperson p.settled_p').css('display', 'block');
		})
		$('.settled_legalperson .input').blur(function () {
			name('.settled_legalperson');
		})

		// 企业法人邮箱
		$('.settled_legalpersonEail .input').focus(function () {
			$('.settled_legalpersonEail p.settled_p').css('display', 'block');
		})
		$('.settled_legalpersonEail .input').blur(function () {
			mail('.settled_legalpersonEail')
		})

		// 企业法人手机号码
		$('.settled_legalpersonPhone .input').focus(function () {
			$('.settled_legalpersonPhone p.settled_p').css('display', 'block');
		})
		$('.settled_legalpersonPhone .input').blur(function () {
			phone('.settled_legalpersonPhone');
		})

		// 再次输入企业法人手机号码
		$('.settled_relegalpersonPhone .input').focus(function () {
			$('.settled_relegalpersonPhone p.settled_p').css('display', 'block');
		})
		$('.settled_relegalpersonPhone .input').blur(function () {
			rePhone('.settled_legalpersonPhone', '.settled_relegalpersonPhone');
		})

		//企业法人身份证号
		$('.settled_legalpersonidentity .input').focus(function () {
			$('.settled_legalpersonidentity p.settled_p').css('display', 'block');
		})
		$('.settled_legalpersonidentity .input').blur(function () {
			identity('.settled_legalpersonidentity');
		})

		// 公司名称
		$('.settled_companyName .input').focus(function () {
			$('.settled_companyName p.settled_p').css('display', 'block');
		})
		$('.settled_companyName .input').blur(function () {
			name('.settled_companyName');
		})

		// 公司地址
		$(".settled_companyaddress .input").focus(function () {
			$('.settled_companyaddress p.settled_p').css('display', 'block');
		})
		$('.settled_companyaddress .input').blur(function () {
			Null('.settled_companyaddress');
		})
	...
	function formImg() {
		...
		// 当input表单字段发生改变时
		$('.settled_representativIDz input.imginput').change(function () {
			cardPositive('.settled_representativIDz');
		})
		$('.settled_representativIDf input.imginput').change(function () {
			cardPositive('.settled_representativIDf');
		})
		$('.settled_openlicense input.imginput').change(function () {
			cardPositive('.settled_openlicense');
		})
		$('.settled_openingpermit input.imginput').change(function () {
			cardPositive('.settled_openingpermit');
		})
		$('.settled_inspectionreport input.imginput').change(function () {
			cardPositive('.settled_inspectionreport');
		})
		$('.settled_BusinessNoteProve input.imginput').change(function () {
			cardPositive('.settled_BusinessNoteProve');
		})
		$('.settled_Brandcertification input.imginput').change(function () {
			cardPositive('.settled_Brandcertification');
		})
	}

	// 下拉菜单函数
	function initMenuListener(box, boxUl, boxli) {
		var input = $(box + ' input');
		var ul = $(boxUl);
		var li = $(boxli);
		ul.css('display', 'none');
	...
	$(function () {
		...
		formImg();
		//转载下拉菜单
		initMenuListener('.settled_category', '.settled_category .category_box', '.category_item')
	})